// src/IracingLiveCoach.App/WeatherWidget.xaml.cs
using System;
...
/// <summary>F1-style combined weather/wetness/precipitation/track-usage widget. Same drag/resize/lock
/// mechanics as every other overlay widget window (see RelativeOverlayWindow's own comment for why
/// they're duplicated rather than shared).</summary>
public partial class WeatherWidget : Window
{
    private readonly WidgetLayout _layout;
    private readonly Action _onChanged;
    private readonly WeatherWidgetViewModel _viewModel = new();

    public IntPtr Handle => new WindowInteropHelper(this).Handle;

    public WeatherWidget(WidgetLayout layout, Action onChanged)
    {
        InitializeComponent();
        _layout = layout;
    ...
    public void UpdateStatus(WeatherStatus status) => _viewModel.Apply(status);
