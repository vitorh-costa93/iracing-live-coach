// src/IracingLiveCoach.App/TireWearWidget.xaml.cs
using System;
...
/// <summary>F1-style tire wear widget. Same drag/resize/lock mechanics as every other overlay
/// widget window (see RelativeOverlayWindow's own comment for why they're duplicated rather than
/// shared).</summary>
public partial class TireWearWidget : Window
{
    private readonly WidgetLayout _layout;
    private readonly Action _onChanged;
    private readonly TireWearWidgetViewModel _viewModel = new();

    public IntPtr Handle => new WindowInteropHelper(this).Handle;

    public TireWearWidget(WidgetLayout layout, Action onChanged)
    {
        InitializeComponent();
        _layout = layout;
    ...
    public void UpdateStatus(TireWearStatus status) => _viewModel.Apply(status);
